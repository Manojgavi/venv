        ...
        for link in links:
            ...
            if url.startswith('/') or url.startswith(self.root_url):
                ...
                if url not in self.scraped_pages:
                    if not url.endswith(".pdf") and "news" not in url and "video" not in url and "wiki" not in url and "google" not in url and "facebook" not in url:
...
class Scraper:
    ...
    def run_scrape(self):
        for page in self.pages:
            ...
            s.run_scraper()
            writer.writerow([page[0], s.emails, len(s.emails)])
            print("Completed " + page[0])


if __name__ == '__main__':
    s = Scraper('deplinks1-250.csv')
    s.read_file()
    s.run_scrape()
